    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-primary text-[#1a1b26] rounded-lg hover:bg-primary/80 transition-colors font-medium"
        >
          <Plus size={18} />
          Add Server
        </button>
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-secondary border border-border rounded-lg text-foreground placeholder-[#565f89] focus:outline-none focus:border-ring"
          />
        </div>
      </div>
      ...
      {showAddForm && (
        <div className="p-4 bg-secondary border border-border rounded-lg">
          <h3 className="text-foreground font-semibold mb-3">Add MCP Server</h3>
          <form onSubmit={handleAddPlugin} className="space-y-3">
            <div>
              <label className="block text-sm text-secondary-foreground mb-1">Server Name</label>
              <input
                type="text"
                value={newPlugin.name}
                onChange={(e) => setNewPlugin({ ...newPlugin, name: e.target.value })}
                placeholder="e.g., filesystem"
                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground placeholder-[#565f89] focus:outline-none focus:border-ring"
                required
              />
            </div>

            <div>
              <label className="block text-sm text-secondary-foreground mb-1">Command or URL</label>
              <input
                type="text"
                ...
                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground placeholder-[#565f89] focus:outline-none focus:border-ring font-mono text-sm"
                required
              />
            </div>

            <div>
              <label className="block text-sm text-secondary-foreground mb-1">Arguments (JSON array)</label>
              <textarea
                value={newPlugin.args}
                onChange={(e) => setNewPlugin({ ...newPlugin, args: e.target.value })}
                placeholder='e.g., ["--arg1", "value1", "--arg2", "value2"]'
                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-foreground placeholder-[#565f89] focus:outline-none focus:border-ring font-mono text-sm h-20 resize-none"
              />
            </div>

            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm text-secondary-foreground">Environment Variables</label>
                <button
                  type="button"
                  onClick={handleAddEnvVar}
                  className="text-xs px-2 py-1 bg-primary text-[#1a1b26] rounded hover:bg-primary/80 transition-colors"
                >
                  + Add Variable
                </button>
              </div>
                ...
                {Object.entries(newPlugin.env).map(([key, value]) => (
                  <div key={key} className="flex gap-2 items-center">
                    <input
                      ...
                      value={key}
                      readOnly
                      className="w-1/3 px-2 py-1 bg-background border border-border rounded text-muted-foreground text-sm"
                    />
                    <input
                      type="text"
                      value={value}
                      ...
                      placeholder="Value"
                      className="flex-1 px-2 py-1 bg-background border border-border rounded text-foreground text-sm focus:outline-none focus:border-ring"
                    />
                    <button
                      type="button"
                      onClick={() => handleRemoveEnvVar(key)}
                      className="text-destructive hover:text-[#db4b4b] transition-colors"
                    >
                      <XCircle size={16} />
                    </button>
                  </div>
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="enabled" className="text-sm text-secondary-foreground">Enable on add</label>
            </div>

            <div className="flex gap-2 pt-2">
              <button
                type="submit"
                className="px-4 py-2 bg-primary text-[#1a1b26] rounded-lg hover:bg-primary/80 transition-colors font-medium"
              >
                Add Server
              </button>
              <button
                ...
                className="px-4 py-2 bg-[#414868] text-foreground rounded-lg hover:bg-[#565f89] transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      {/* Stats Footer */}
      {plugins.length > 0 && (
        <div className="flex items-center gap-4 text-sm text-muted-foreground pt-2 border-t border-[#2a2f44]">
          ...
          {onRefreshTools && (
            <button
              ...
              className="ml-auto flex items-center gap-1 text-primary hover:text-[#9aa5ce] transition-colors"
